---
title: "Effect of missingness - four items w/ moderate to very strong correlations"
...
_Average item interrelations:_ moderate (mean(r) = .4); strong (mean(r) = .6); very strong (mean(r) = .8) <br>
...
#correlation matrix
mod_cor_matrix <- rbind(c(1, .45, .5, .3),
                       c(.45, 1, .4, .45),
                       c(.5, .4, 1, .3),
                       c(.3, .45, .3, 1)) %>% as.matrix()

str_cor_matrix <- rbind(c(1, .65, .7, .5),
                        c(.65, 1, .6, .65),
                        c(.7, .6, 1, .5),
                        c(.5, .65, .5, 1)) %>% as.matrix()

vstr_cor_matrix <- rbind(c(1, .85, .9, .7),
                         c(.85, 1, .7, .85),
                         c(.9, .8, 1, .7),
                         c(.7, .85, .7, 1)) %>% as.matrix()

full_cor_matrix <- rbind(mod_cor_matrix, str_cor_matrix, vstr_cor_matrix)
...
for (strength in c("Moderate", "Strong", "Very Strong")) {
  
  if (strength == "Moderate") {
    cor_matrix <- full_cor_matrix[1:4,]
  } else if (strength == "Strong") {
    cor_matrix <- full_cor_matrix[5:8,]
  } else {
    cor_matrix <- full_cor_matrix[9:12,]
  }
  ...
  for (sample_size in sample_size_options) {
    ...
    for (missing in missing_prob) {
      ...
      print(paste0("simulation step: ", strength, ", n = ", sample_size, ", %NA = ", missing, ", iter. = ", iteration))
    ...
    summary_plot_cor <- sim_summary[-1,] %>% 
                          ...
                          ggplot(aes(x = missing, y = med_dev)) +
                                ...
                                ylim(0, 2) +
                                labs(
                                     title = paste0("N = ", sample_size, " (", strength, " Item Intercorrelations)"),
    ...
    #ifelse(cor_matrix == mod_cor_matrix, strength == "mod", 
    #        ifelse(cor_matrix == str_cor_matrix, strength == "strong", strength == "vstrong"))
    
    cor_plot_name <- paste0("plot_cor_", sample_size, "_", strength)
    assign(cor_plot_name, summary_plot_cor)
    
    mean_plot_name <- paste0("plot_mean_", sample_size, "_", strength)
...
ggpubr::ggarrange(plot_cor_5000_Moderate, plot_cor_3000_Moderate, plot_cor_1000_Moderate, 
                  plot_cor_500_Moderate, plot_cor_200_Moderate, 
          ncol = 1, nrow = 5)
```
...
ggpubr::ggarrange(plot_mean_5000_Moderate, plot_mean_3000_Moderate, plot_mean_1000_Moderate, 
                  plot_mean_500_Moderate, plot_mean_200_Moderate, 
          ncol = 1, nrow = 5)
```
...
ggpubr::ggarrange(plot_cor_5000_Strong, plot_cor_3000_Strong, plot_cor_1000_Strong, 
                  plot_cor_500_Strong, plot_cor_200_Strong, 
          ncol = 1, nrow = 5)
```
...
ggpubr::ggarrange(plot_mean_5000_Strong, plot_mean_3000_Strong, plot_mean_1000_Strong, 
                  plot_mean_500_Strong, plot_mean_200_Strong, 
          ncol = 1, nrow = 5)
```

# Very Strong Intercorrelations 

## Outcome 1: Difference bet. Correlations

```{r fig.width=6, fig.height=17, warning=FALSE, fig.align='center'}

ggpubr::ggarrange(`plot_cor_5000_Very Strong`, `plot_cor_3000_Very Strong`, `plot_cor_1000_Very Strong`, 
                  `plot_cor_500_Very Strong`, `plot_cor_200_Very Strong`, 
          ncol = 1, nrow = 5)
```
...
ggpubr::ggarrange(`plot_mean_5000_Very Strong`, `plot_mean_3000_Very Strong`, `plot_mean_1000_Very Strong`, 
                  `plot_mean_500_Very Strong`, `plot_mean_200_Very Strong`, 
          ncol = 1, nrow = 5)
```